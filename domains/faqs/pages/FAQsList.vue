  ...
  <div class="row g-5 g-xl-8 mb-5 mb-xl-8" v-if="!faqsStore.isLoading">
        ...
        <div class="card card-flush h-xl-100">
          <div class="card-body">
            <span class="svg-icon svg-icon-2tx svg-icon-primary">
              <i class="ki-duotone ki-question fs-2tx text-primary">
      ...
      <div class="col-xl-3">
        <div class="card card-flush h-xl-100">
          <div class="card-body">
            ...
            <div class="fw-bold fs-2 mt-5">
              {{ Object.keys(faqsStore.statistics.byCategory).length }}
            </div>
            <div class="fw-semibold text-muted">
              Categories
            </div>
    ...
    <div class="card">
        ...
        <div class="card-toolbar">
          <div class="d-flex justify-content-end gap-2" data-kt-customer-table-toolbar="base">
            ...
            <select
              ...
              class="form-select form-select-solid w-150px"
              v-if="faqsStore.categories.length > 0"
            >
              <option :value="null">All Categories</option>
              <option
                v-for="category in faqsStore.categories"
                :key="category"
                :value="category"
              >
                {{ category }}
      ...
      <div class="card-body pt-0">
        <!-- Table -->
        <div class="table-responsive" v-if="!faqsStore.isLoading && faqsStore.hasFAQs">
          <table class="table align-middle table-row-dashed fs-6 gy-5">
            ...
            <tbody class="fw-semibold text-gray-600">
              <tr v-for="faq in faqsStore.filteredFAQs" :key="faq.id">
                <td>
                ...
                <td>
                  <span class="text-muted fs-7">
                    {{ getAnswerPreview(faq.answer, 100) }}
                  </span>
                </td>
                <td>
                  <span
        ...
        <!-- Table Skeleton -->
        <div class="table-responsive" v-else-if="faqsStore.isLoading">
          <table class="table align-middle table-row-dashed fs-6 gy-5">
            <thead>
              <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
        ...
        >
          ...
          <NuxtLink to="/faqs/create" class="btn btn-primary">
            <i class="ki-duotone ki-plus fs-2"></i>
            Create FAQ
          </NuxtLink>
        </div>
...
const allSelected = computed(() => {
  return (
    faqsStore.filteredFAQs.length > 0 &&
    selectedIds.value.length === faqsStore.filteredFAQs.length
  )
})
...
const toggleSelectAll = () => {
  if (allSelected.value) {
    ...
    selectedIds.value = faqsStore.filteredFAQs.map((f) => f.id)
  }
}
...
const handleSearch = async () => {
  await faqsStore.debouncedSearch(searchQuery.value)
}

const handleStatusFilter = async () => {
  await faqsStore.applyStatusFilter(statusFilter.value)
}

const handleCategoryFilter = async () => {
  await faqsStore.applyCategoryFilter(categoryFilter.value)
}

const handleDelete = async (faq: FAQ) => {
  await confirmAndDeleteFAQ(faq)
}

const handleToggleStatus = async (faq: FAQ) => {
  await toggleFAQStatus(faq)
}

const handleBulkDelete = async () => {
  const success = await bulkDeleteFAQs(selectedIds.value)
  if (success) {
    selectedIds.value = []
  }
}
