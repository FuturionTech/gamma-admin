<template>
  <!-- Page Header -->
  <PageHeader
    :title="`Edit: ${banner?.title || 'Banner'}`"
    subtitle="Modify banner information"
  />
    ...
    <!-- Edit Form -->
    <div v-else-if="banner" class="row g-5">
      <!-- Form Card -->
      <div class="col-lg-8">
        <div class="card">
          <div class="card-body">
...
const { uploadBannerImage, confirmAndDeleteBanner } = useBannersActions()
...
// Form data
const formData = reactive<UpdateBannerInput & { imageFile?: File | null }>({
  title: null,
  subtitle: null,
  image_url: null,
  cta_text: null,
...
const bannerPreviewStyle = computed(() => ({
  backgroundImage: `url(${formatImageUrl(formData.image_url || banner.value?.image_url)})`,
...
const loadBanner = async () => {
  ...
  try {
    ...
    if (loadedBanner) {
      ...
      // Populate form
      formData.title = loadedBanner.title
      formData.subtitle = loadedBanner.subtitle
      formData.image_url = loadedBanner.image_url
      formData.cta_text = loadedBanner.cta_text
      formData.cta_url = loadedBanner.cta_url
      formData.order = loadedBanner.order
      formData.is_active = loadedBanner.is_active

      // Enable CTA if both text and URL exist
      ctaEnabled.value = !!(loadedBanner.cta_text && loadedBanner.cta_url)

      // Update breadcrumb
      breadcrumbStore.setBreadcrumb([
        { title: 'Home', path: '/' },
        { title: 'Banners', path: '/banners' },
        { title: loadedBanner.title, path: `/banners/${bannerId}/edit` }
      ])
    }
  } catch (error: any) {
    showError(error.message || 'Failed to load banner')
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await bannersStore.updateBanner(bannerId, input)
